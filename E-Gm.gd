...
@export var menprt:JSON                     ##partida
@export var mjugad:Array[Node]              ##jugadores
@export var tablee:Array[Node]              ##tableros de la esena
...
func _ready() -> void:
	if Engine.is_editor_hint():
		...
		if len(tabler)>0 and !"rTb" in RR:RR["rTb"]=[1,0,""]
		if !"MM" in RR:RR["MM"]=[]
		if len(RR["MM"])==0 and len(mjugad)>0:RR["MM"].append(mjugad[0].entblr)
		if !"TL" in RR:RR["TL"]=[89,11,6,4,15,12]
		if !"TT" in RR:RR["TT"]=[0 ,1 ,0,0,0 ,0 ]
		R["T"]=0
		EgLb.cg+="G"
func _process(_delta: float) -> void:
	if Engine.is_editor_hint():
		...
		#		if cnp:
		#			if len(RR["MM"][x].enbolt)>0:
		#				EgLb.atlz(RR["MM"][x],RR["MM"][x].R["evEm"])
		#				RR["MM"][x].R["evEm"]+=1
		#				if RR["MM"][x].R["evEm"]>=len(RR["MM"][x].enbolt):
		#					RR["MM"][x].R["evEm"]=0
		#					RR["MM"][x].enbolt=EgLb.barj(RR["MM"][x].enbolt)
		#		x+=1
